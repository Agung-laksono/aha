    ...
    <div
        ...
        <div
                    ...
                    @if($searchBarangResults->isNotEmpty())
                        <div
                            ...
                            @foreach($searchBarangResults as $res)
                                <button type="button" @click="soundInput.play()" wire:click="addToPurchaseCart({{ $res->id }})"
            ...
            <div class="flex-1 flex justify-end items-center gap-2 sm:gap-3">
                ...
                <!-- Close Button -->
                <button type="button" @click="soundLoadMore.play()" wire:click="$set('showPurchaseModal', false)"
        ...
        <div class="flex-grow flex overflow-hidden">
            ...
            <div
                class="w-full w-1/1 tablet-vertical:w-[60%] md:w-2/3 flex flex-col p-6 overflow-y-auto custom-scrollbar border-r dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                <div class="space-y-6">
                    <!-- Cart Table Area -->
                    <div class="space-y-4">
                        <div class="flex items-center justify-between px-2">
                            ...
                            @if(count($purchaseCart) > 0)
                                <button type="button" @click="soundWrong.play()" wire:click="clearPurchaseCart"
                        ...
                        <div class="space-y-4">
                            @forelse($purchaseCart as $index => $item)
                                                    <div x-data="{ rowActive: false }"
                                                        ...
                                                        <button type="button" @click="soundOutPut.play()" wire:click="removeFromPurchaseCart({{ $index }})"
                                                            title="Hapus Item"
                                                                         ...
                                                                         @if($item['status'] === 'Received')
                                                                             <div class="flex gap-2">
                                                                                 <select wire:model.live="purchaseCart.{{ $index }}.gudang_id"
                                                                                     class="flex-grow text-xs font-black uppercase border-2 border-gray-100 dark:border-gray-700 rounded-xl p-2.5 dark:bg-gray-800 bg-white focus:ring-0 focus:border-primary-500 shadow-sm transition-all cursor-pointer text-gray-700 dark:text-gray-300 @error('purchaseCart.' . $index . '.gudang_id') border-rose-500 focus:border-rose-500 @enderror">
                                                                                 ...
                                                                                 @can('manage_gudang')
                                                                                     <button type="button" data-modal-target="modal-gudang" data-modal-toggle="modal-gudang"
                                                                                         class="flex-shrink-0 p-2.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 border-2 border-emerald-100 dark:border-emerald-800/50 rounded-xl hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all shadow-sm active:scale-95"
            ...
            <div
                class=" w-full tablet-vertical:w-[40%] md:w-1/3 flex flex-col bg-gray-50/50 dark:bg-gray-900/30 p-6 overflow-y-auto custom-scrollbar border-l dark:border-gray-700">
                <div class="space-y-8">
                    <!-- SECTION: VENDOR -->
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest">Informasi
                                Vendor</label>
                            @if($selectedVendor)
                                <button type="button" @click="soundOutPut.play()" wire:click="$set('selectedVendor', null)"
                                ...
                                <div class="relative flex-grow" x-data="{ open: false }">
                                    ...
                                    <div x-show="open"
                                        ...
                                        @forelse($vendors as $v)
                                            <button type="button" @click="soundInput.play()" @mousedown.prevent
