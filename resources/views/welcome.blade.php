...
<body
    class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] flex p-6 lg:p-8 items-center lg:justify-center min-h-screen flex-col bg-cover bg-center bg-no-repeat bg-fixed"
    style="background-image: url('{{ asset('storage/asset/bg-utama.png') }}');">
    {{-- Dynamic Processor Toggle Layout --}}
    <div x-data="{ showLogin: false }" class="relative flex-1 flex items-center justify-center w-full px-6">

        {{-- Processor Icon (Initially Visible) --}}
        <a href="{{ route('login') }}"
        <button x-show="!showLogin" x-transition:enter="transition ease-out duration-500"
            x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100"
            x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 scale-100"
            x-transition:leave-end="opacity-0 scale-110" @click="showLogin = true"
            class="group relative flex items-center justify-center p-8 bg-white/10 dark:bg-gray-900/10 backdrop-blur-md rounded-[3rem] border border-white/20 dark:border-gray-700/30 hover:bg-white/20 dark:hover:bg-gray-800/20 transition-all duration-500 hover:shadow-2xl hover:shadow-orange-600/20 active:scale-95">
            <div
                class="absolute inset-0 bg-orange-600/20 rounded-[3rem] blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            </div>
            <svg class="w-6 h-6 text-white group-hover:text-orange-500 transition-colors duration-500" fill="none"
                stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                    d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z" />
            </svg>
            <span
                class="absolute -bottom-10 text-white/50 font-bold tracking-widest uppercase text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-500">Press
                to Initialize</span>
        </button>
        </a>

        {{-- Navigation Card (Initially Hidden) --}}
        <div x-show="showLogin" x-cloak x-transition:enter="transition ease-out duration-700 delay-300"
            x-transition:enter-start="opacity-0 translate-y-8 blur-lg"
            x-transition:enter-end="opacity-100 translate-y-0 blur-0"
            class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl p-10 lg:p-14 rounded-[2.5rem] shadow-2xl border border-white/20 dark:border-gray-700/30 text-center max-w-lg w-full flex flex-col items-center">
            <nav class="flex flex-col gap-4 w-full">
                @auth
                    <a href="{{ url('/dashboard') }}"
                        class="w-full py-5 px-8 bg-orange-600 hover:bg-orange-700 text-white rounded-2xl font-black text-xl shadow-2xl shadow-orange-600/30 transition-all flex items-center justify-center gap-3 active:scale-95">
                ...
                @else
                    <a href="{{ route('login') }}"
                        class="w-full py-5 px-8 bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:bg-black dark:hover:bg-gray-100 rounded-2xl font-black text-xl shadow-2xl shadow-black/20 dark:shadow-white/10 transition-all active:scale-95">
                        Masuk Sekarang
                    </a>

                    @if (Route::has('register'))
                        <a href="{{ route('register') }}"
                            class="w-full py-5 px-8 bg-white/40 dark:bg-gray-800/40 hover:bg-white/60 dark:hover:bg-gray-800/60 text-gray-700 dark:text-gray-200 border-2 border-gray-100 dark:border-gray-700 rounded-2xl font-black text-xl transition-all active:scale-95">
                            Daftar Akun
                        </a>
                    @endif
                @endauth
            ...
            <button @click="showLogin = false"
                class="mt-8 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors text-sm font-bold flex items-center gap-2">
